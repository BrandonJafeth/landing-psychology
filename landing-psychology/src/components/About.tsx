...
const About = () => {
  ...
  useGSAP(() => {
    // 1. Background Shapes (Infinite)
    gsap.to('.bg-shape-1', { scale: 1.1, duration: 4, repeat: -1, yoyo: true, ease: "sine.inOut" });
    gsap.to('.bg-shape-2', { scale: 1.05, duration: 5, repeat: -1, yoyo: true, ease: "sine.inOut", delay: 1 });
    ...
    // 3. Floating Badge
    gsap.to('.floating-badge', { y: -10, duration: 2, repeat: -1, yoyo: true, ease: "sine.inOut" });

    // 4. Content Column Entrance
    gsap.fromTo('.content-column',
      { y: 50, opacity: 0 },
      {
        y: 0,
        opacity: 1,
    ...
    // 5. Modalities Cards Hover
    const cards = gsap.utils.toArray('.modality-card');
    cards.forEach((card: any) => {
      card.addEventListener('mouseenter', () => {
        gsap.to(card, { y: -5, boxShadow: "0 10px 30px -10px rgba(0,0,0,0.1)", duration: 0.3 });
      });
  ...
  return (
    <section ref={container} id="sobre-mi" className="relative w-full py-12 md:py-20 lg:py-32 bg-white overflow-hidden">
      <div className="container mx-auto px-6 md:px-12">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          ...
          <div className="w-full lg:w-1/2 relative flex justify-center lg:justify-end min-h-[500px] md:min-h-[600px]">
            ...
            <div 
              className="bg-shape-1 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] md:w-[500px] md:h-[500px] bg-[#61B39C]/10 rounded-full blur-3xl z-0"
            />
            
            <div 
              className="bg-shape-2 absolute top-10 -right-10 w-40 h-40 bg-[#559A95]/5 rounded-full blur-2xl z-0"
            />
            
            {/* Image Container */}
            <div 
              className="image-container relative z-10 flex items-end justify-center lg:justify-end w-full max-w-[500px] lg:max-w-[550px]"
            >
              {/* Main Image Wrapper */}
                ...
                {/* Floating Badge */}
                <div 
                  className="floating-badge absolute bottom-4 left-4 md:left-8 bg-white p-4 md:p-5 rounded-2xl shadow-xl border border-gray-100 max-w-[170px] z-20"
                >
            ...
            <div className="content-column">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
                {about.modalities.map((item, index) => (
                  <div 
                    key={index}
                    className={`modality-card p-6 rounded-2xl border border-gray-100 transition-all duration-300 ${item.color} bg-opacity-40`}
                  >
